...
HWND VulkanInvaders::hWnd;
HINSTANCE VulkanInvaders::hInstance;
WNDPROC VulkanInvaders::wndProc;
#endif
...
void VulkanInvaders::createSwapchain()
{
...
VulkanInvaders::VulkanInvaders()
{
	...
	#if defined(VK_USE_PLATFORM_WIN32_KHR)
	{
		...
		wndClass.lpfnWndProc = VulkanInvaders::wndProc;
		wndClass.cbClsExtra = 0;
		wndClass.cbWndExtra = 0;
		wndClass.hInstance = VulkanInvaders::hInstance;
		...
		VulkanInvaders::hWnd = CreateWindowEx(0, "VulkanInvaders", "Vulkan Invaders", dwStyle, 0, 0, windowRect.right - windowRect.left, windowRect.bottom - windowRect.top, NULL, NULL, VulkanInvaders::hInstance, NULL);

		if (!fullscreen) {
			uint32_t x = (GetSystemMetrics(SM_CXSCREEN) - windowRect.right) / 2;
			uint32_t y = (GetSystemMetrics(SM_CYSCREEN) - windowRect.bottom) / 2;
			SetWindowPos(VulkanInvaders::hWnd, 0, x, y, 0, 0, SWP_NOZORDER | SWP_NOSIZE);
		}

		ShowWindow(VulkanInvaders::hWnd, SW_SHOW);
		SetForegroundWindow(VulkanInvaders::hWnd);
		SetFocus(VulkanInvaders::hWnd);
	...
	// Surface
	{
		...
		surfaceCreateInfo.hinstance = VulkanInvaders::hInstance;
		surfaceCreateInfo.hwnd = VulkanInvaders::hWnd;
...
void VulkanInvaders::updateCommandBuffers()
{
